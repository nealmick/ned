...
TerminalPanel::TerminalPanel() : impl_(std::make_unique<Impl>()) { impl_->addSession(); }

TerminalPanel::~TerminalPanel() { shutdown(); }

void TerminalPanel::setProjectRoot(const std::string &root) { impl_->projectRoot = root; }

bool TerminalPanel::visible() const { return impl_->visible; }

bool TerminalPanel::isStarted() const
{
...
void TerminalPanel::hide() { setVisible(false); }
...
void TerminalPanel::toggle() { setVisible(!impl_->visible); }

void TerminalPanel::setVisible(bool on, bool focus)
{
...
bool TerminalPanel::consumeNeedsFontResync()
{
