...
bool AppHost::initialize()
{
	...
	glfwSetScrollCallback(window_, AppHost::scrollCallback);

	initialized_ = true;
	return true;
}

void AppHost::scrollCallback(GLFWwindow *window, double xoffset, double yoffset)
{
	AppHost *ned = static_cast<AppHost *>(glfwGetWindowUserPointer(window));
	if (!ned)
		return;
...
void AppHost::handleScrollAccumulators()
{
