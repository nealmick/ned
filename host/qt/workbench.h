...
class EditorGroup : public QTabWidget
{
  ...
  Q_SIGNALS:
	// DnD relay — Workbench owns the zone logic and performs the move.
...
class Workbench : public QWidget
{
  ...
  public:
	...
	void addEditor(EditorFrame *view, const QString &title, bool focus);

	// Tab bookkeeping used by the host shell.
	QList<EditorFrame *> views() const;
	int editorCount() const;
	EditorGroup *groupForView(const EditorFrame *view) const;
	EditorFrame *viewForPath(const QString &path) const;
	// Editor in the focused group's current tab (null when none).
	EditorFrame *activeView() const;
  ...
  Q_SIGNALS:
	...
	void editorClosed(EditorFrame *view);
  ...
  private:
	...
	struct DropHint
	{
		...
		QRect preview;		  // highlight rect in Workbench coordinates
	...
	void moveView(EditorFrame *view, EditorGroup *to, int index); // -1 = append
