/*
	File: host/qt/terminal_panel.cpp
	Description: TerminalPanel implementation — multi-session QTermWidget
...
struct TerminalPanel::Impl
{
	...
	// shared factory (tab_close.h) keeps size/sheet/hover identical.
...
TerminalPanel::TerminalPanel(QWidget *parent) : QWidget(parent), impl_(new Impl)
{
	impl_->q = this;
	...
	// one intended border (see the #terminalTabs rule in theme.h).
	...
	// and install the same button (active tab only, Workbench style).
...
void TerminalPanel::applyFont(const QFont &font)
{
	impl_->font = font;
	for (auto &s : impl_->sessions)
