...
bool Ned::initialize()
{
	...
	if (!gSettings.isAgentSplitPosProcessed() && !showSidebar)
	{
		...
		// When sidebar is hidden, the agent split position represents editor width, not
		// agent pane width So we need to invert it: 1 - saved_agent_position
...
void Ned::renderResizeHandles()
{
	...
	ImGui::InvisibleButton("##resize_right",
						   ImVec2(resizeBorder, windowSize.y)); // Will now be 100px wide
	...
	ImGui::InvisibleButton("##resize_bottom",
						   ImVec2(windowSize.x, resizeBorder)); // Will now be 100px high
	...
	ImGui::InvisibleButton("##resize_corner",
						   ImVec2(resizeBorder, resizeBorder)); // Will now be 100x100px
...
ImFont *Ned::loadFont(const std::string &fontName, float fontSize)
{
	...
	// std::cout << "[Ned::loadFont] Attempting to load font from: " << fontPath << " at
	// size "
	//		  << fontSize << std::endl;
	...
	// std::cout << "[Ned::loadFont] Successfully loaded font: " << fontName << " from "
	// << fontPath
...
void Ned::handleKeyboardShortcuts()
{
	...
	if (modPressed && ImGui::IsKeyPressed(toggleSidebar, false))
	{
		...
		float availableWidth =
			...
			(showAgentPane
				 ? kAgentSplitterWidth
				 : 0.0f); // Only account for splitter width when agent pane is visible
...
void ApplySettings(ImGuiStyle &style)
{
	...
	// ImGui::GetIO().FontGlobalScale = gSettings.getSettings()["fontSize"].get<float>()
	// / 16.0f;
}