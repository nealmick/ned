...
bool LSPGotoDef::gotoDefinition(const std::string &filePath, int line, int character) {
    ...
    std::cout << "\033[31mDEBUG:\033[0m Python file detected, proceeding with goto definition" << std::endl;

    // Select adapter for this file and initialize if needed
    if (!gLSPManagerWindows.selectAdapterForFile(filePath)) {
        std::cout << "\033[33mLSP GotoDef Windows:\033[0m No adapter available for file: " << filePath << std::endl;
        return false;
    }
    ...
    try {
        ...
        if (j.contains("result")) {
            ...
            if (result.is_array()) {
                ...
                // Take the first definition (primary definition)
                auto firstDef = result[0];
                if (firstDef.contains("uri") && firstDef.contains("range")) {
                    std::string uri = firstDef["uri"].get<std::string>();
                    auto range = firstDef["range"];
                    ...
                    std::cout << "\033[32mLSP GotoDef Windows:\033[0m Found definition at " 
                              << filePath << ":" << (defLine + 1) << ":" << (defChar + 1) << std::endl;

                    // Navigate to the definition using the same logic as Linux LSP
                    if (filePath != gFileExplorer.currentFile) {
                        // Load different file
                        std::cout << "\033[35mLSP GotoDef Windows:\033[0m Loading file: " << filePath << std::endl;
                        gFileExplorer.loadFileContent(filePath, [defLine, defChar]() {
                            std::cout << "\033[32mLSP GotoDef Windows:\033[0m File loaded, setting cursor position..." << std::endl;
                            // Don't mess with scroll centering - it might interfere with input focus
                            // Just set basic cursor position after file loads
                            int index = 0;
                            int currentLine = 0;
                            
                            while (currentLine < defLine && index < editor_state.fileContent.length()) {
                            ...
                            // File loaded and cursor positioned successfully
                        });
                    } else {
                        // Same file, just move cursor
                        std::cout << "\033[35mLSP GotoDef Windows:\033[0m Navigating within current file..." << std::endl;
                        int index = 0;
                        int currentLine = 0;
                        ...
                        index = std::min(index, (int)editor_state.fileContent.length());
                        editor_state.cursor_index = index;
                        // Don't mess with scrolling/centering - it might interfere with input focus
                        
                        std::cout << "\033[32mLSP GotoDef Windows:\033[0m Cursor moved to position " << index << std::endl;
                    }
            ...
            // Handle single Location object
                ...
                std::cout << "\033[32mLSP GotoDef Windows:\033[0m Found single definition at " 
                          << filePath << ":" << (defLine + 1) << ":" << (defChar + 1) << std::endl;
                ...
                // Clear any stuck input state by resetting editor focus
                // Note: editor_state.has_focus doesn't exist in EditorState
