...
bool LSPAdapterClangd::initialize(const std::string &workspacePath)
{
    ...
    try {
        std::cout << "\033[35mClangd:\033[0m Starting LSP server with path: " << lspPath << std::endl;
        ...
        std::cout << "\033[35mClangd:\033[0m Sending initialize request for workspace: " << workspacePath << std::endl;
...
{
    if (!impl->input) {
        std::cerr << "\033[31mClangd:\033[0m Cannot send request - server not initialized" << std::endl;
        return false;
    }
...
{
    if (!impl->output) {
        std::cerr << "\033[31mClangd:\033[0m Cannot read response - server not initialized" << std::endl;
        return "";
    }
