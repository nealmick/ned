...
bool LSPManager::initialize(const std::string &path)
{
    ...
    case CLANGD:
        ...
        if (success) {
            ...
            std::cerr << "\033[31mLSP Manager:\033[0m Failed to initialize clangd adapter" << std::endl;
        }
        break;
    ...
    case PYRIGHT:
        ...
        if (success) {
            ...
            std::cerr << "\033[31mLSP Manager:\033[0m Failed to initialize pyright adapter" << std::endl;
        }
        break;
...
bool LSPManager::selectAdapterForFile(const std::string &filePath)
{
    ...
    if (dot_pos == std::string::npos) {
        std::cout << "\033[33mLSP Manager:\033[0m File has no extension, defaulting to no adapter" << std::endl;
        activeAdapter = NONE;
        return false;
    }
    ...
    if (ext == "c" || ext == "cpp" || ext == "cc" || ext == "cxx" || ext == "h" || ext == "hpp") {
        std::cout << "\033[32mLSP Manager:\033[0m Selected clangd adapter for file: " << filePath << std::endl;
        activeAdapter = CLANGD;
        return true;
    } else if (ext == "py") {
        std::cout << "\033[32mLSP Manager:\033[0m Selected pyright adapter for file: " << filePath << std::endl;
        activeAdapter = PYRIGHT;
        return true;
    }

    // No appropriate adapter found
    std::cout << "\033[33mLSP Manager:\033[0m No adapter available for extension: " << ext << std::endl;
    activeAdapter = NONE;
    return false;
}
