#include "qt_lsp_view.h"

#include "../../../editor/util/doc_path.h"
#include "../../../editor/views/qt/ned_key_qt.h"
...
LspQtView::LspQtView(LSPClient &clientIn,
					 Settings &settingsIn,
...
{
	picker = new QtLspUriOptions(hostWindow);
	// Enter/double-click in the picker: jump in place, or open the target
	// document and jump there.
	connect(
		picker, &QtLspUriOptions::locationSelected, this, [this](const LSPLocation &loc) {
			jumpTo(loc);
		});
	dashboard_ = new QtLspDashboard(
		client,
		...
		hostWindow);
	symbolInfo = new QtLspSymbolInfo(client, settings, this);
...
LspQtView::~LspQtView() { qApp->removeEventFilter(this); }

void LspQtView::editorOpened(QtEditorView &view)
{
...
void LspQtView::rebind(QtEditorView *view)
{
	active = view;
...
bool LspQtView::eventFilter(QObject *watched, QEvent *event)
{
	...
	{
		if (auto *view = qobject_cast<QtEditorView *>(watched); view && view == active)
			rebind(nullptr);
		return false;
	}
	...
	if (type == QEvent::MouseMove && symbolInfo->isVisible() &&
		qobject_cast<QtEditorView *>(watched))
		symbolInfo->dismiss();

	if (type != QEvent::KeyPress)
		return false;
	auto *key = static_cast<QKeyEvent *>(event);
	auto *view = qobject_cast<QtEditorView *>(watched);
	if (!view)
		return false;
...
bool LspQtView::handleKeybinds(QtEditorView &view, QKeyEvent *event)
{
...
void LspQtView::jumpTo(const LSPLocation &location)
{
	QtEditorView *target = active;
	if (!target)
		return;
