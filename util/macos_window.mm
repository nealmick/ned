...
- (NSApplicationTerminateReply)applicationShouldTerminate:(NSApplication *)sender {
    // Use the normal clean shutdown path for all termination methods, including
    // Cmd+Q. The main loop detects termination via shouldTerminateApplication()
    // and calls Ned::cleanup(), which tears down ImGui last. Calling exit(0)
    // here previously killed the process during global destruction and crashed
    // with a null ImGui context (GetIO assertion).
...
void configureMacOSWindow(void* window, float opacity, bool blurEnabled) {
    ...
    CGFloat cornerRadius = 14.0f; // Default for older versions
