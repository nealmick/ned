...
void TreeSitter::parse(const std::string &fileContent, std::vector<ImVec4> &fileColors, const std::string &extension)
{
...
void TreeSitter::traverse(const std::string &extension, const std::string &fileContent, std::vector<ImVec4> &fileColors, TSNode node, int depth, bool is_last, const std::vector<bool> &hierarchy, const std::string &parentNodeType)
{
    ...
    std::cout << COLOR_POS << "Idx " << start_byte << "-" << end_byte << COLOR_RESET << " " << prefix << " color: " << int(color.x * 255) << "," << int(color.y * 255) << "," << int(color.z * 255) << ": " << type << " -- " << content << std::endl;
    ...
    for (uint32_t i = 0; i < child_count; i++) {
        ...
        traverse(extension, fileContent, fileColors, child, depth + 1, last_child, child_hierarchy, type);
    }
}

const ImVec4 &TreeSitter::convertNodeTypeJS(const std::string &nodeType, const std::string &parentNodeType, const std::string &nodeText)
{
    static const std::unordered_map<std::string, ImVec4> baseMap = {{"string", cachedColors.string}, {"number", cachedColors.number}, {"comment", cachedColors.comment}, {"string_fragment", cachedColors.string}, {"jsx_text", cachedColors.text}, {"template_string", cachedColors.string}, {"true", cachedColors.keyword}, {"false", cachedColors.keyword}, {"null", cachedColors.keyword}};
    ...
    if ((parentNodeType == "jsx_opening_element" || parentNodeType == "jsx_closing_element") && nodeType == "identifier") {
    ...
    static const std::unordered_set<std::string> keywords = {"return", "function", "if", "else", "for", "while", "do", "switch", "case", "try", "catch", "finally", "throw", "new", "delete", "typeof", "instanceof", "void", "break", "continue", "debugger", "var", "let", "const", "class", "extends", "super", "this", "async", "await", "yield", "of", "in", "static", "get", "set", "export", "import", "default", "from", "as", "typeof", "interface", "type", "implements", "namespace"};
    ...
    // Function handling
    if (nodeType == "function_declaration" || nodeType == "arrow_function" || nodeType == "method_definition" || nodeType == "call_expression") {
        return cachedColors.function;
    }

    // React hooks pattern
    if (nodeType == "identifier" && parentNodeType == "lexical_declaration") {
        static const std::unordered_set<std::string> hooks = {"useState", "useEffect", "useContext", "useReducer", "useCallback", "useMemo", "useRef", "useImperativeHandle", "useLayoutEffect"};
...
const ImVec4 &TreeSitter::convertNodeTypeCPP(const std::string &nodeType)
{
    ...
    static const std::unordered_map<std::string, ImVec4 &> cppTypeMap = {{"number_literal", cachedColors.number}, {"string_literal", cachedColors.string}, {"char_literal", cachedColors.string}, {"comment", cachedColors.comment}, {"function_definition", cachedColors.function}, {"call_expression", cachedColors.function}, {"field_identifier", cachedColors.function}, {"primitive_type", cachedColors.keyword}, {"type_identifier", cachedColors.keyword}, {"if", cachedColors.keyword}, {"for", cachedColors.keyword}, {"while", cachedColors.keyword}, {"return", cachedColors.keyword}, {"class_specifier", cachedColors.keyword}, {"namespace_identifier", cachedColors.function}};
