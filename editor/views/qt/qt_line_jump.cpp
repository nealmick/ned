...
QtLineJumpDialog::QtLineJumpDialog(int currentLine, int lines, QWidget *parent)
	: QDialog(parent, Qt::Popup), lineCount(lines)
{
	...
	// two-layer pattern as QtFileFinder (opaque card on a clear popup).
...
void QtLineJumpDialog::keyPressEvent(QKeyEvent *event)
{
	switch (event->key())
	{
	case Qt::Key_Escape:
