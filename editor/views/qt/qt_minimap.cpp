...
QtMinimap::Geometry
QtMinimap::geometry(qreal stripH, int viewLines, int lineCount, int maxScrollLine)
{
	Geometry g;
...
qreal QtMinimap::Geometry::sliderTopFor(qreal scrollLines) const
{
	return std::clamp(scrollLines * ratio, 0.0, maxTop);
}

qreal QtMinimap::Geometry::scrollLinesForY(qreal yInStrip) const
{
	if (ratio <= 0.0)
		return 0.0;
...
void QtMinimap::ensureRuns(const QString &newKey,
						   int startRow,
						   int endRow,
						   EditorState &state,
