...
class QtEditorView : public QWidget, public LspEditor
{
	Q_OBJECT
  ...
  public:
	...
	// minimap is on. Strip geometry + density-run cache live in QtMinimap.
  ...
  private:
	...
	// --- Diagnostics (LSP) -------------------------------------------------
	// Gutter severity column width (0 when no store is bound) — included in
	// updateGutterWidth so text/marks share one coordinate space.
	int diagnosticColumnWidth() const;
	...
	// --- Hover (VSCode-style trigger shared with LSP symbol hover) --------
	HoverTrigger::Target hoverTargetAt(const QPoint &pos) const;
	// Feed the trigger and dispatch transitions: diagnostic tooltips show
	// directly; Text cells without diagnostics go to hoverObserver.
	...
	WrapLayout wrap;
	QtMinimap minimap; // density-run cache + strip geometry (qt_minimap.h)
	...
	void paintTitleStrip(QPainter &painter);
	...
	qreal paintText(QPainter &painter, int firstRow, int rows, qreal yBase);
	void paintSelectionsAndCarets(
