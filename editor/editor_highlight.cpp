...
void EditorHighlight::highlightContent(bool fullRehighlight, bool sync)
{
    ...
    auto performHighlighting = [this, content_copy, extension_copy, fullRehighlight](std::vector<ImVec4>& colors) {
        try
        {
    ...
    if (sync) {
        ...
        highlightFuture = std::async(
            std::launch::async,
            [this, content_copy, colors_param_copy, currentFile_copy, performHighlighting]() mutable {
