...
bool Editor::textEditor(const char *label, std::string &text, std::vector<ImVec4> &colors, EditorState &editor_state) {
    ...
    ImVec2 text_pos;
    beginTextEditorChild(label, remaining_width, content_width, content_height,
                         current_scroll_y, current_scroll_x, text_pos,
                         editor_top_margin, text_left_margin, editor_state);
...
ImVec2 Editor::renderLineNumbersPanel(float line_number_width, float editor_top_margin) {
    ...
    ImGui::BeginChild("LineNumbers", ImVec2(line_number_width, ImGui::GetContentRegionAvail().y),
...
void Editor::beginTextEditorChild(const char* label, float remaining_width, float content_width, float content_height,
                                  float &current_scroll_y, float &current_scroll_x, ImVec2 &text_pos,
                                  float editor_top_margin, float text_left_margin, EditorState &editor_state) {
    ...
    ImGui::BeginChild(label, ImVec2(remaining_width, ImGui::GetContentRegionAvail().y),
...
void Editor::processTextEditorInput(std::string &text, EditorState &editor_state, const ImVec2 &text_start_pos,
                                    float line_height, bool &text_changed, std::vector<ImVec4> &colors,
                                    CursorVisibility &ensure_cursor_visible, int initial_cursor_pos) {
...
void Editor::adjustScrollForCursorVisibility(const ImVec2 &text_pos, const std::string &text, EditorState &editor_state,
                                               float line_height, float window_height, float window_width,
                                               float &current_scroll_y, float &current_scroll_x, CursorVisibility &ensure_cursor_visible) {
...
void Editor::renderEditorContent(const std::string &text, const std::vector<ImVec4> &colors, EditorState &editor_state,
                                   float line_height, const ImVec2 &text_pos) {
...
void Editor::updateFinalScrollAndRenderLineNumbers(const ImVec2 &line_numbers_pos, float line_number_width, float editor_top_margin,
                                                     const ImVec2 &size, EditorState &editor_state, float line_height, float total_height) {
    ...
    ImGui::PushClipRect(line_numbers_pos,
                        ImVec2(line_numbers_pos.x + line_number_width, line_numbers_pos.y + size.y - editor_top_margin),
                        true);
    renderLineNumbers(line_numbers_pos, line_number_width, line_height, editor_state.line_starts.size(),
                      editor_state.scroll_pos.y, size.y - editor_top_margin, editor_state, editor_state.cursor_blink_time);
    ImGui::PopClipRect();
    ImGui::EndGroup();
    ImGui::PopID();
}
...
void Editor::endSelection(EditorState &state) { 
    state.is_selecting = false; 
}

int Editor::getSelectionStart(const EditorState &state) {
    return std::min(state.selection_start, state.selection_end);
}

int Editor::getSelectionEnd(const EditorState &state) {
...
float Editor::calculateCursorXPosition(const ImVec2 &text_pos, const std::string &text,
                               int cursor_pos) {
  float x = text_pos.x;
...
ScrollChange Editor::ensureCursorVisible(const ImVec2 &text_pos,
                                 const std::string &text, EditorState &state,
                                 float line_height, float window_height,
                                 float window_width) {
  float cursor_y =
      (gEditor.getLineFromPos(state.line_starts, state.cursor_pos) * line_height);
  ...
  if (cursor_x < visible_start_x + buffer) {
    ...
    state.scroll_x =
        cursor_x - text_pos.x - buffer * 2; // Double buffer on left
    changed.horizontal = true;
  } else if (cursor_x >
             visible_end_x - buffer * 1.5f) { // Start scrolling earlier
    // Scrolling right - immediately jump
    state.scroll_x =
        cursor_x - window_width + buffer * 2; // More space on right
    changed.horizontal = true;
  }
...
void Editor::cutSelectedText(std::string &text, std::vector<ImVec4> &colors,
...
void Editor::cutWholeLine(std::string &text, std::vector<ImVec4> &colors,
                  EditorState &state, bool &text_changed) {
...
void Editor::pasteText(std::string &text, std::vector<ImVec4> &colors,
               EditorState &state, bool &text_changed) {
...
void Editor::handleMouseInput(const std::string &text, EditorState &state,
                      const ImVec2 &text_start_pos, float line_height) {
  static bool is_dragging = false;
  static int drag_start_pos = -1;

  ImVec2 mouse_pos = ImGui::GetMousePos();
  int char_index = gEditor.getCharIndexFromCoords(text, mouse_pos, text_start_pos,
...
void Editor::cursorUp(const std::string &text, EditorState &state,
                  float line_height, float window_height) {
...
void Editor::cursorDown(const std::string &text, EditorState &state,
                    float line_height, float window_height) {
  int current_line = gEditor.getLineFromPos(state.line_starts, state.cursor_pos);
...
void Editor::moveCursorVertically(std::string &text, EditorState &state,
                          int line_delta) {
  int current_line = gEditor.getLineFromPos(state.line_starts, state.cursor_pos);
  int target_line =
...
void Editor::handleCursorMovement(const std::string &text, EditorState &state,
...
// Text input handling
void Editor::handleCharacterInput(std::string &text, std::vector<ImVec4> &colors,
                          EditorState &state, bool &text_changed,
...
void Editor::handleEnterKey(std::string &text, std::vector<ImVec4> &colors,
...
void Editor::handleDeleteKey(std::string &text, std::vector<ImVec4> &colors,
                     EditorState &state, bool &text_changed, int &input_end) {
...
void Editor::handleBackspaceKey(std::string &text, std::vector<ImVec4> &colors,
                        EditorState &state, bool &text_changed,
                        int &input_start) {
...
void Editor::handleTabKey(std::string &text, std::vector<ImVec4> &colors,
                  EditorState &state, bool &text_changed, int &input_end) {
...
void Editor::handleTextInput(std::string &text, std::vector<ImVec4> &colors,
                     EditorState &state, bool &text_changed) {
  ...
  if (text_changed) {
    ...
    int line_start =
        state.line_starts[gEditor.getLineFromPos(state.line_starts, input_start)];

    // Get the end of the line where the change ended (or the end of the text if
    // it's the last line)
    int line_end =
        input_end < text.size()
            ? state.line_starts[gEditor.getLineFromPos(state.line_starts, input_end)]
            : text.size();
...
// Rendering functions
void Editor::renderTextWithSelection(ImDrawList *drawList, const ImVec2 &pos,
      ...
      if (should_highlight) {
        ...
        drawList->AddRectFilled(
            ...
            ImGui::ColorConvertFloat4ToU32(
                ImVec4(1.0f, 0.1f, 0.7f, 0.3f))); // Pink with 30% alpha
      }
...
void Editor::renderCursor(ImDrawList *draw_list, const ImVec2 &cursor_screen_pos,
...
int Editor::getCharIndexFromCoords(const std::string &text, const ImVec2 &click_pos,
...
void Editor::renderLineNumbers(const ImVec2 &pos, float line_number_width,
  ...
  const ImU32 selected_line_color =
      IM_COL32(0, 40, 255, 200); // Neon pink color
  int start_line = static_cast<int>(scroll_y / line_height);
  int end_line =
      std::min(num_lines,
  ...
  for (int i = start_line; i < end_line; i++) {
    ...
    if (i >= selection_start_line && i < selection_end_line &&
      ...
      line_number_color =
          selected_line_color; // Use neon pink for selected lines
...
float Editor::calculateTextWidth(const std::string &text,
                         const std::vector<int> &line_starts) {
...
void Editor::handleEditorInput(std::string &text, EditorState &state,
                               const ImVec2 &text_start_pos, float line_height,
...
bool Editor::validateHighlightContentParams(const std::string &content,
                                              const std::vector<ImVec4> &colors,
...
void Editor::highlightContent(const std::string &content,
                              ...
                              int end_pos) {
    std::cout << "\033[36mEditor:\033[0m   Highlight Content. content size: "
    ...
    highlightFuture = std::async(std::launch::async, [this, content, &colors, start_pos, end_pos, extension]() {
        try {
            if (extension == ".cpp" || extension == ".h") {
                ...
                // No syntax highlighting available; use default colors.
