...
class Lexer
{
  public:
    ...
    struct ThemeColors
    {
        ImVec4 keyword, string, number, comment, text, function, componentName, reactHook, operatorColor, returnBlock;
    };
        ...
        while (pos < codeLen && iterations < maxIterations) {
            ...
            case LexerState::JS: {
                ...
                if (currentToken.length > 0) {
                    ...
                    if (currentToken.type == TokenType::Keyword && code.substr(currentToken.start, currentToken.length) == "return") {
                        ...
                        if (nextCharPos < codeLen && (code[nextCharPos] == '(' || code[nextCharPos] == '{' || code[nextCharPos] == '<')) {
                            // Consume the whitespace between 'return' and the starter
                            if (nextCharPos > pos) {
                            ...
                            // 'continue' to let the next iteration handle the '(', '{', or '<' in the new state
                            continue;
                        }
                        // If not followed by starter, remain in JS state. 'pos' is already correct.
                    }
                } else if (currentToken.type != TokenType::Unknown) {
                    // lexJS might have just changed state (e.g., to TEMPLATE_LITERAL) without consuming chars.
                    // Let the next iteration handle it.
                } else {
                    // Lexer stall in JS state
                    if (pos < codeLen) {
                        // std::cerr << "Warning: Lexer stall in JS state at pos " << pos << ". Consuming char '" << code[pos] << "'" << std::endl;
            ...
            case LexerState::IN_RETURN_BLOCK: {
                ...
                currentToken = lexReturnBlock(code, pos, stateStack); // stateStack is popped inside lexReturnBlock
                if (currentToken.length > 0) {
                    tokens.push_back(currentToken);
                    lastNonWsToken = currentToken.type;
                } else {
                    // Error or EOF within block, state already popped. Let JS handle next.
                    if (pos < codeLen) {
                        // std::cerr << "Warning: lexReturnBlock returned 0 length. Pos: " << pos << std::endl;
                        // Let outer loop handle potential stall if pos hasn't advanced
                    }
                }
            ...
            case LexerState::TEMPLATE_LITERAL: {
                currentToken = lexTemplateLiteral(code, pos, stateStack); // Handles state changes internally
                if (currentToken.length > 0) {
                    tokens.push_back(currentToken);
                    lastNonWsToken = currentToken.type;
                } else if (currentToken.type != TokenType::Unknown) {
                    ...
                    if (pos < codeLen) {
                        // std::cerr << "Warning: Lexer stall in Template Literal state at pos " << pos << ". Consuming char '" << code[pos] << "'" << std::endl;
                        tokens.push_back({TokenType::Unknown, pos, 1});
                        pos++;
                    }
                    // Pop state if stalled at end? lexTemplateLiteral should handle this.
                }
    ...
    { /* ... unchanged ... */
        if (colors.size() < code.length()) {
            ...
            std::fill(colors.begin(), colors.begin() + code.length(), getColorForTokenType(TokenType::Unknown));
        }
        try {
  ...
  private:
    ...
    { /* ... unchanged ... */
        ...
        cachedColors.operatorColor = loadColor("operator", ImVec4(cachedColors.text.x * 0.8f, cachedColors.text.y * 0.8f, cachedColors.text.z * 0.8f, cachedColors.text.w));
    ...
    // Lexes standard JavaScript (mostly unchanged, handles Template Literal state)
    Token lexJS(const std::string &code, size_t &pos, std::stack<LexerState> &stateStack, TokenType lastNonWsToken)
    {
        size_t start = pos;
        if (pos >= code.length())
        ...
        if (c == '`') {
            ...
            if (stateStack.size() > 1) {
                ...
                if (parentState == LexerState::TEMPLATE_LITERAL) {
                    // Pop the actual JS state we just put back, the caller (lexTemplateLiteral) manages the actual pop later
                    stateStack.pop();
    ...
    Token lexReturnBlock(const std::string &code, size_t &pos, std::stack<LexerState> &stateStack)
    {
        ...
        if (pos >= code.length()) { // Safety check at start
        ...
        if (start_char == '(') {
            ...
            std::cerr << "Error: lexReturnBlock unexpected start char '" << start_char << "' at pos " << pos << std::endl;
            if (!stateStack.empty() && stateStack.top() == LexerState::IN_RETURN_BLOCK)
                stateStack.pop();
            return {TokenType::Unknown, block_start, 0};
        }
        ...
        while (pos < code.length()) {
            ...
            if (c == '/' && pos + 1 < code.length()) {
                if (code[pos + 1] == '/') { /* Skip Single Line Comment */
                    pos += 2;
                    while (pos < code.length() && code[pos] != '\n')
                        pos++;
                    if (pos < code.length())
                        pos++;
                    continue;
                } else if (code[pos + 1] == '*') { /* Skip Multi Line Comment */
                    pos += 2;
                    bool closed = false;
                    while (pos + 1 < code.length()) {
                        if (code[pos] == '*' && code[pos + 1] == '/') {
            ...
            if (c == '`') { /* Skip Template Literal */
                ...
                while (pos < code.length()) {
                    if (code[pos] == '\\' && pos + 1 < code.length())
                        ...
                        pos += 2;
                        int templ_balance = 1;
                        while (pos < code.length() && templ_balance > 0) { /* Skip nested structures inside ${} */
            ...
            if (seek_jsx_end) {
                ...
                if (c == '}' && internal_brace_balance < 0 && internal_paren_balance == 0) {
                    // Correction: check internal_brace_balance < 0, as the closing '}'
                    // brings the count below zero if it's the outer function's brace.
                    // DO NOT consume the '}' - it belongs to the outer JS scope.
                    break; // Found likely end of containing block
                }
            } else {                   // Balancing '(' or '{'
                if (c == start_char) { // Needs to match the specific opener '(' or '{'
                    balance++;
                } else if (c == end_char) { // Needs to match the specific closer ')' or '}'
                    balance--;
                    if (balance == 0) {
        ...
        if (!stateStack.empty() && stateStack.top() == LexerState::IN_RETURN_BLOCK) {
            ...
            // std::cerr << "Warning: Popping state but top was not IN_RETURN_BLOCK at pos " << pos << std::endl;
        }
        ...
        while (pos < code.length()) {
            if (code[pos] == '\\' && pos + 1 < code.length()) {
                ...
                if (pos > start) {
                    ...
                    // Pop state *after* returning this token (or before if returning final backtick)
                    Token textToken = {TokenType::TemplateString, start, pos - start};
                    // State popped by caller or next iteration detecting the ` token below
                    return textToken;
                } else {
                    // End of literal right away (empty literal `` or just processed ${...})
                    pos++;
