...
class Lexer
{
  public:
    ...
    Lexer()
    {
        keywords = {"abstract", "as", "base", "break", "case", "catch", "checked", "class", "const", "continue", "default", "delegate", "do", "else", "enum", "event", "explicit", "extern", "finally", "fixed", "for", "foreach", "goto", "if", "implicit", "in", "interface", "internal", "is", "lock", "namespace", "new", "operator", "out", "override", "params", "private", "protected", "public", "readonly", "ref", "return", "sealed", "sizeof", "stackalloc", "static", "struct", "switch", "this", "throw", "try", "typeof", "unchecked", "unsafe", "using", "virtual", "volatile", "while", "add", "alias", "ascending", "async", "await", "by", "descending", "dynamic", "equals", "from", "get", "global", "group", "into", "join", "let", "nameof", "on", "orderby", "partial", "remove", "select", "set", "value", "var", "when", "where", "yield", "unmanaged", "nint", "nuint", "notnull", "and", "or", "not", "record", "init", "with", "managed"};
        builtInTypes = {"bool", "byte", "sbyte", "char", "decimal", "double", "float", "int", "uint", "nint", "nuint", "long", "ulong", "short", "ushort", "object", "string", "void", "dynamic"};
        literals = {"true", "false", "null"};
        operators = {{">>=", TokenType::Operator}, {"<<=", TokenType::Operator}, {"==", TokenType::Operator}, {"!=", TokenType::Operator}, {">=", TokenType::Operator}, {"<=", TokenType::Operator}, {"&&", TokenType::Operator}, {"||", TokenType::Operator}, {"??", TokenType::Operator}, {"?.", TokenType::Operator}, {"=>", TokenType::Operator}, {"++", TokenType::Operator}, {"--", TokenType::Operator}, {"+=", TokenType::Operator}, {"-=", TokenType::Operator}, {"*=", TokenType::Operator}, {"/=", TokenType::Operator}, {"%=", TokenType::Operator}, {"&=", TokenType::Operator},
                     {"|=", TokenType::Operator},  {"^=", TokenType::Operator},  {"::", TokenType::Operator}, {"<<", TokenType::Operator}, {">>", TokenType::Operator}, {"+", TokenType::Operator},  {"-", TokenType::Operator},  {"*", TokenType::Operator},  {"/", TokenType::Operator},  {"%", TokenType::Operator},  {"=", TokenType::Operator},  {">", TokenType::Operator},  {"<", TokenType::Operator},  {"!", TokenType::Operator},  {"&", TokenType::Operator},  {"|", TokenType::Operator},  {"^", TokenType::Operator},  {"~", TokenType::Operator},  {"?", TokenType::Operator}};
        ...
        try {
            while (pos < code.length() && iterations < maxIterations) {
                ...
                if (pos > wsStart) {
                    ...
                    atLineStart = (pos == lineStart); // Re-check if line start after whitespace
                    if (pos >= code.length())
                        break;
                ...
                } else if (current_char == '$' && pos + 1 < code.length() && (code[pos + 1] == '"' || (code[pos + 1] == '@' && pos + 2 < code.length() && code[pos + 2] == '"'))) {
                    tokens.push_back(lexInterpolatedString(code, pos));
                } else if (current_char == '@' && pos + 1 < code.length() && code[pos + 1] == '$' && pos + 2 < code.length() && code[pos + 2] == '"') {
                    tokens.push_back(lexInterpolatedString(code, pos));
                } else if (current_char == '"') {
                    tokens.push_back(lexString(code, pos));
                } else if (current_char == '\'') {
                    tokens.push_back(lexCharLiteral(code, pos));
                } else if (isCSharpIdentifierStart(current_char) || (current_char == '@' && pos + 1 < code.length() && isCSharpIdentifierStart(code[pos + 1]))) { // Handle regular and @ identifiers
  ...
  private:
    ...
    bool isAttributeContext(const std::string &code, size_t pos, const std::vector<Token> &tokens) const
    {
        if (pos == 0)
            return false;
        ...
        static const std::unordered_set<std::string> contextKeywords = {"public", "private", "protected", "internal", "static", "abstract", "sealed", "virtual", "override", "new", "async", "unsafe", "class", "struct", "interface", "enum", "delegate", "event", "void"};
    ...
    Token lexNumber(const std::string &code, size_t &pos)
    {
        size_t s = pos;
        bool isF = false;
        if (code[pos] == '0' && pos + 1 < code.length() && (code[pos + 1] == 'x' || code[pos + 1] == 'X')) {
            pos += 2;
            while (pos < code.length() && (isHexDigit(code[pos]) || code[pos] == '_'))
                pos++;
        } else if (code[pos] == '0' && pos + 1 < code.length() && (code[pos + 1] == 'b' || code[pos + 1] == 'B')) {
            pos += 2;
            while (pos < code.length() && (code[pos] == '0' || code[pos] == '1' || code[pos] == '_'))
                pos++;
        } else {
    ...
    Token lexIdentifierKeywordOrType(const std::string &code, size_t &pos, const std::vector<Token> &tokens)
    {
        ...
        if (pTI != -1) {
            ...
            if (t.type == TokenType::Keyword) {
                if (pW == "class" || pW == "interface" || pW == "struct" || pW == "enum" || pW == "delegate" || pW == "record" || pW == "namespace")
