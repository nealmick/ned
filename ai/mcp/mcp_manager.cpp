...
std::string Manager::executeToolCall(const std::string& toolName, const std::unordered_map<std::string, std::string>& parameters) {
    ...
    try {
        if (toolName == "listFiles") {
            ...
            if (it != parameters.end()) {
                ...
                std::string result = success ? "File created successfully: " + it->second : "Failed to create file: " + it->second;
                return result;
            }
        }
            ...
            if (it != parameters.end()) {
                ...
                std::string result = success ? "Folder created successfully: " + it->second : "Failed to create folder: " + it->second;
                return result;
            }
        }
            ...
            if (targetFileIt != parameters.end() && instructionsIt != parameters.end() && codeEditIt != parameters.end()) {
                std::string result = fileSystemServer.editFile(targetFileIt->second, instructionsIt->second, codeEditIt->second);
                return result;
            }
        }
