...
void AIAgentTextInput::setSendMessageCallback(std::function<void(const char*, bool)> callback) {
...
void AIAgentTextInput::setNotificationCallback(std::function<void(const char*, float)> callback) {
...
void AIAgentTextInput::renderInputBox(const ImVec2& textBoxSize, float textBoxWidth) {
    ...
    if (isFocused && state) {
        ...
        did_wrap = HandleWordBreakAndWrap(inputBuffer, bufferSize, state, textBoxWidth - 10.0f, textBoxWidth);
    }
...
bool AIAgentTextInput::HandleWordBreakAndWrap(char* inputBuffer, size_t bufferSize, ImGuiInputTextState* state, float max_width, float text_box_x) {
