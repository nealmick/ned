...
void Ned::run()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		handleFileDialog();
		renderFrame();
		fontManager.handleFontReload(needFontReload);
...
void Ned::renderEditorHeader(ImFont *currentFont)
{
	...
	ImGui::BeginGroup();
	ImGui::PushFont(fontManager.getCurrentFont());
...
{
	gAIAgent.render(agentPaneWidth, fontManager.getLargeFont());
}
...
void Ned::renderMainWindow()
{
	...
	if (showSidebar)
	{
		...
		// Render Editor
		renderEditor(fontManager.getCurrentFont(), editorWidth);
		if (showAgentPane)
		{
			ImGui::SameLine(0, 0);
...
void Ned::handleUltraSimpleResizeOverlay()
{
	...
	if (m_sroStartTime > 0.0 && elapsedTime < displayDuration)
	{
		...
		ImFont *font =
			fontManager.getLargeFont(); // Use the large font instead of scaling
